// Admin pages exports
export { default as Dashboard } from './Dashboard';
export { default as Procurement } from './Procurement';
export { default as DryingProcess } from './DryingProcess';
export { default as Processing } from './Processing';
export { default as Products, Varieties, Inventory } from './Products';
export { default as Sales } from './Sales';
export { default as Partners, Supplier, Client } from './Partners';
export { default as StaffManagement } from './StaffManagement';
export { default as AdminOrders } from './Orders';
export { default as AuditTrail } from './AuditTrail';
export { default as Archives } from './Archives';
export { default as Settings } from './Settings';
